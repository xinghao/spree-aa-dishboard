  ...
  module Aa
    ...
    class DashboardController < Spree::Admin::BaseController
      def index
        ...
        @orders = Spree::Order.where("(state = 'complete' and payment_state != 'paid') or (((state != 'complete' and state != 'canceled') or completed_at is null) and payment_state = 'paid' and total > 0)").order("completed_at asc").all 
